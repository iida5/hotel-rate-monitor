...
export class RateShopMonitor {
  ...
  applyScope(scopePrefixes, observed, ts, { initial = false, quiet = false, meta = null } = {}) {
    ...
    for (const [key, cell] of observed) {
      ...
      // ① プローブが一時失敗で在庫推定が不確定なときは前回の在庫値を引き継ぐ。
      //    (プローブ途中の fetch 失敗で推定が低く出る「谷」を作らず、誤った増減アラートを防ぐ)
      if (cell.stockUncertain && prev && prev.stock != null) {
        cell.stock = prev.stock; cell.stockPlus = prev.stockPlus; cell.stockSrc = prev.stockSrc
      }
      // 在庫増減の判定基準値 (前回アラートした確定値)。初回・旧データは現在値で初期化。
      const baseStock = prev?.stockBaseline ?? cell.stock
      const baseStockPlus = prev?.stockBasePlus ?? !!cell.stockPlus
      cell.stockBaseline = baseStock; cell.stockBasePlus = baseStockPlus // 既定は基準据え置き
      const stored = { ...cell, t: ts } // t = 最終観測時刻 (鮮度表示用)
      ...
      if (!prev.avail && cell.avail) {
        stored.stockBaseline = cell.stock; stored.stockBasePlus = !!cell.stockPlus // 販売再開で基準リセット
        fresh.push(this.alert('restock', ts, key, cell.plan, { ...meta, price: cell.price, stock: cell.stock, stockPlus: !!cell.stockPlus, stockSrc: cell.stockSrc ?? null, planName: cell.planName }))
        continue
      }
      if (prev.avail && !cell.avail) {
      ...
      // ② 在庫増減はヒステリシスで確定させる: 同じ値が2回連続で観測 (held) され、かつ
      //    基準値と異なるときだけアラート。単発の谷/山 (プローブ推定ノイズ) は無視する。
      //    在庫はプローブ推定 (stockSrc='probe') が主で値が揺れやすいための保険。
      if (cell.stock != null && prev.stock != null) {
        const held = cell.stock === prev.stock // 2回連続で同値 = 確定
        if (held && baseStock != null && cell.stock !== baseStock) {
          fresh.push(this.alert(cell.stock > baseStock ? 'stock_up' : 'stock_down', ts, key, cell.plan, {
            ...meta, oldStock: baseStock, oldStockPlus: baseStockPlus,
            stock: cell.stock, stockPlus: !!cell.stockPlus, stockSrc: cell.stockSrc ?? null,
            price: cell.price,
          }))
          stored.stockBaseline = cell.stock; stored.stockBasePlus = !!cell.stockPlus // 基準を確定値に更新
        } else if (held) {
          stored.stockBaseline = cell.stock; stored.stockBasePlus = !!cell.stockPlus // 同値確定で基準維持
        }
      }
    }
